import { Controller, Delete, HttpCode, HttpStatus, Param, ParseIntPipe, Req } from '@nestjs/common';
...
export class CidadeControllerRemove {
  ...
  async remove(@Req() res: Request, @Param('id', ParseIntPipe) id: number): Promise<Result<void>> {
    const _link = gerarLinks(res, CIDADE.ENTITY);
    await this.cidadeServiceRemove.remove(id);
    return MensagemSistema.showMensagem(HttpStatus.OK, 'Cidade excluída com sucesso!', null, res.path, null, _link);
  }
}
